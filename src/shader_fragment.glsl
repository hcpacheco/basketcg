...
//        V = (position_model.y - miny) / (maxy - miny);
//
//        vec3 Kd2 = texture(TextureImage5, vec2(U,V)).rgb;
//
        float lambert = max(0,dot(n,l));
//        color = Kd2 * (lambert + 0.01);
          color = vec3(0.2f,0.2f,0.6f) * (lambert*0.9);
    }



//    if(lambert > 0)
//        color = Kd0 * (lambert + 0.01) + Kd1 * (1 - lambert) * 0.2;
//    else
//        color = Kd0 * (lambert + 0.01) + Kd1 * (1 - lambert) * 0.5;
