...
// renderização.
float g_CameraTheta = -2.5f; // Ângulo no plano ZX em relação ao eixo Z
...
int main(int argc, char* argv[])
{
    ...
    FramebufferSizeCallback(window, 1000, 1000); // Forçamos a chamada do callback acima, para definir g_ScreenRatio.
    ...
    ObjModel cubev("../../data/cubev.obj");
    ComputeNormals(&cubev);
    BuildTrianglesAndAddToVirtualScene(&cubev);

    ObjModel spheremodel("../../data/sphere.obj");
    ComputeNormals(&spheremodel);
    BuildTrianglesAndAddToVirtualScene(&spheremodel);

    ObjModel cestamodel("../../data/cesta.obj");
...
//      alterado
        if(lookAt){
            ...
            //camera_position_cb = glm::vec4(0.0f, -20.0f,0.0f,1.0f);
...
void ComputeNormals(ObjModel* model)
{
    ...
    for (size_t shape = 0; shape < model->shapes.size(); ++shape)
    {
        ...
        for (size_t triangle = 0; triangle < num_triangles; ++triangle)
        {
            ...
            glm::vec4  a = vertices[0];
            glm::vec4  b = vertices[1];
            glm::vec4  c = vertices[2];
